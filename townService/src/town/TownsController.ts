...
export class TownsController extends Controller {
  ...
  /**
   * Creates a presentation area in a given town
   *
   * @param townID ID of the town in which to create the new presentation area
   * @param sessionToken session token of the player making the request, must
   *        match the session token returned when the player joined the town
   * @param requestBody The new presentation area to create
   *
   * @throws InvalidParametersError if the session token is not valid, or if the
   *          presentation area could not be created
   */
  @Post('{townID}/presentationArea')
  @Response<InvalidParametersError>(400, 'Invalid values specified')
  public async createPresentationArea(
    @Path() townID: string,
    @Header('X-Session-Token') sessionToken: string,
    @Body() requestBody: PresentationArea,
  ): Promise<void> {
    const town = this._townsStore.getTownByID(townID);
    if (!town) {
      throw new InvalidParametersError('Invalid values specified');
    ...
    const success = town.addPresentationArea(requestBody);
    if (!success) {
      throw new InvalidParametersError('Invalid values specified');
    }
  }
